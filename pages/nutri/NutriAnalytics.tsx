...
import { nutriApi, NutriWeightLog, NutriDailyLog, NutriMeal, NutriDietPlan, MEAL_TYPES } from '../../lib/api-nutri';
...
export default function NutriAnalytics() {
    ...
    const loadData = async () => {
        setLoading(true);
        const [w, dl, m, dp] = await Promise.all([
    ...
    // Water chart data
    const waterChartData = filteredDaily.map(d => ({
    ...
    const avgWater = filteredDaily.filter(d => d.water_ml).length > 0 ? Math.round(filteredDaily.filter(d => d.water_ml).reduce((s, d) => s + (d.water_ml || 0), 0) / filteredDaily.filter(d => d.water_ml).length) : 0;
        ...
        <div className="min-h-screen flex flex-col pb-28">
            ...
            <main className="flex-1 px-5 space-y-4 animate-enter">
                ...
                <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="nutri-card rounded-2xl p-3 text-center">
                        <p className="text-lg font-bold text-sky-400">{avgWater}ml</p>
                        <p className="text-[9px] text-white/30 font-bold uppercase mt-0.5">Água/Dia</p>
                    </div>
                    <div className="nutri-card rounded-2xl p-3 text-center">
                ...
                {/* Water Chart */}
                <ChartCard title="Hidratação" icon="water_drop" iconColor="#38bdf8" isEmpty={waterChartData.length < 2} emptyMsg="Registre a água consumida">
                    <ResponsiveContainer width="100%" height={160}>
                        <BarChart data={waterChartData}>
                            ...
                            <Bar dataKey="agua" fill="#38bdf8" radius={[4, 4, 0, 0]} opacity={0.7} />
                        </BarChart>
                    </ResponsiveContainer>
                </ChartCard>
                ...
                <ChartCard title="Nível de Energia" icon="bolt" iconColor="#fbbf24" isEmpty={energyChartData.length < 2} emptyMsg="Registre seu nível de energia">
                    <ResponsiveContainer width="100%" height={140}>
                        <LineChart data={energyChartData}>
                            ...
                            <Tooltip {...tooltipStyle} />
                            <Line type="monotone" dataKey="energia" stroke="#fbbf24" strokeWidth={2.5} dot={{ r: 3, fill: '#fbbf24', strokeWidth: 0 }} />
                        </LineChart>
                    </ResponsiveContainer>
                </ChartCard>
